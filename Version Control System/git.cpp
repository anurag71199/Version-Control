...
void rollback();
void diffUtil(string, string);
void diff(vector<string>);
void log();
...
void commitUtil(string cwd, vector<string> &pversion_files, unordered_map<string, string> &statusFiles)
{
    if (countDirectories("./.git/version") != 0)
    {
        // Find the files in that directory
        string folder = "./.git/version/v_" + to_string(countDirectories("./.git/version"));
        chdir(&folder[0]);
        vector<string> files;
        ...
        chdir(&folder[0]);
        string dest = "../v_" + to_string(countDirectories("./.git/version") + 1);
...
void commit()
{
    ...
    string destination = "./.git/version/v_" + to_string(countDirectories("./.git/version") + 1);
    ...
    commitUtil(cwd, pversion_files, statusFiles); // new
...
bool compareVersions(dirent* a, dirent* b) {
	if(string(a->d_name) == string(b->d_name)) return false;
	else return true;
}
...
void deleteFolder(vector<string> arr)
{
    ...
    {
        if (arr[i] == "." || arr[i] == ".." || arr[i] == "git" || arr[i] == ".git" || arr[i] == "git.cpp" || arr[i] == "sha1.hpp")
            continue;
        string path = arr[i];
        // Directory delete
        if (isDirExist(path))
...
void diffUtil(string path, string t)
{
    struct stat fileInfo;
    if (!stat(&path[0], &fileInfo)) // run on stat==0
    {
        string temp = "diff -y --suppress-common-lines ";
        string mo = temp + path + " " + t;
        system(&mo[0]);
    }
    return;
}
void diff(vector<string> allFiles)
{
    string destination = "./.git/version/v_" + to_string(countDirectories("./.git/version")) + "/";
    for (int i = 0; i < allFiles.size(); i++)
    {
        int dif;
        string path = destination + allFiles[i];
        cout << endl;
        cout << "FILE NAME: " << path << endl;
        cout << "==============================================" << endl;
        diffUtil(path, allFiles[i]);
        cout << "==============================================" << endl;
        cout << endl;
    }
    return;
